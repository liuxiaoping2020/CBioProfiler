...
getCGPinfo<-function (drug, tissueType = "all", dataset = "cgp2014")
{
  ...
  if (dataset == "cgp2014") {
    ...
    possibleDrugs <- c("A.443654", "A.770041", "ABT.263",
                       ...
                       "BI.2536", "BIBW2992", "Bicalutamide", "BI.D1870",
                       "BIRB.0796", "Bleomycin", "BMS.509744", "BMS.536924",
                       "BMS.708163", "BMS.754807", "Bortezomib", "Bosutinib",
                       "Bryostatin.1", "BX.795", "Camptothecin", "CCT007093",
                       "CCT018159", "CEP.701", "CGP.082996", "CGP.60474",
                       "CHIR.99021", "CI.1040", "Cisplatin", "CMK", "Cyclopamine",
                       "Cytarabine", "Dasatinib", "DMOG", "Docetaxel", "Doxorubicin",
                       "EHT.1864", "Elesclomol", "Embelin", "Epothilone.B",
                       "Erlotinib", "Etoposide", "FH535", "FTI.277", "GDC.0449",
                       "GDC0941", "Gefitinib", "Gemcitabine", "GNF.2", "GSK269962A",
    ...
    if (!tissueType %in% c("all", "aero_digestive_tract",
                           ...
                           "nervous_system", "skin", "urogenital_system"))
    ...
    possibleDrugs2016 <- c("Erlotinib", "Rapamycin", "Sunitinib",
                           ...
                           "JW-7-52-1", "A-443654", "GW843682X", "MS-275", "Parthenolide",
                           "KIN001-135", "TGX221", "Bortezomib", "XMD8-85",
                           "Roscovitine", "Salubrinal", "Lapatinib", "GSK269962A",
                           "Doxorubicin", "Etoposide", "Gemcitabine", "Mitomycin C",
                           "Vinorelbine", "NSC-87877", "Bicalutamide", "QS11",
                           "CP466722", "Midostaurin", "CHIR-99021", "AP-24534",
                           "AZD6482", "JNK-9L", "PF-562271", "HG-6-64-1", "JQ1",
                           "JQ12", "DMOG", "FTI-277", "OSU-03012", "Shikonin",
                           ...
                           "GSK690693", "SNX-2112", "QL-XI-92", "XMD13-2", "QL-X-138",
                           "XMD15-27", "T0901317", "EX-527", "THZ-2-49", "KIN001-270",
                           "THZ-2-102-1", "AICAR", "Camptothecin", "Vinblastine",
                           "Cisplatin", "Cytarabine", "Docetaxel", "Methotrexate",
                           "ATRA", "Gefitinib", "Navitoclax", "Vorinostat",
                           "Nilotinib", "RDEA119", "CI-1040", "Temsirolimus",
                           "Olaparib", "Veliparib", "Bosutinib", "Lenalidomide",
                           "Axitinib", "AZD7762", "GW 441756", "CEP-701", "SB 216763",
                           "17-AAG", "VX-702", "AMG-706", "KU-55933", "Elesclomol",
                           "Afatinib", "GDC0449", "PLX4720", "BX-795", "NU-7441",
                           "SL 0101-1", "BIRB 0796", "JNK Inhibitor VIII", "681640",
                           "Nutlin-3a (-)", "PD-173074", "ZM-447439", "RO-3306",
                           "MK-2206", "PD-0332991", "BEZ235", "GDC0941", "AZD8055",
                           "PD-0325901", "SB590885", "selumetinib", "CCT007093",
                           "EHT 1864", "Cetuximab", "PF-4708671", "JNJ-26854165",
                           "HG-5-113-01", "HG-5-88-01", "TW 37", "XMD11-85h",
                           "ZG-10", "XMD8-92", "QL-VIII-58", "CCT018159", "AG-014699",
                           "SB 505124", "Tamoxifen", "QL-XII-61", "PFI-1", "IOX2",
                           "YK 4-279", "(5Z)-7-Oxozeaenol", "piperlongumine",
                           "FK866", "Talazoparib", "rTRAIL", "UNC1215", "SGC0946",
                           "XAV939", "Trametinib", "Dabrafenib", "Temozolomide",
                           "Bleomycin (50 uM)", "SN-38", "MLN4924")
...
checkmode<-function(Trainsurv_subset,clinical_subset){
  for(i in 1:ncol(clinical_subset)){
    if(class(Trainsurv_subset[,i])!=class(clinical_subset[,i])){
      return(FALSE)
    }
  }
  return(TRUE)
...
transf<-function(df){
  is_char_or_factor <- sapply(df, function(x) is.character(x) | is.factor(x))
  df_subset_char <- df[, is_char_or_factor,drop = FALSE]
  df_subset_num<-df[,!is_char_or_factor,drop = FALSE]
  # toTitleCase <- function(x){
  #   # Convert to title case
  #   if(x %in% NA){
  #     return(x)
  #   }else{
  #     title_case_string = paste0(toupper(substring(x, 1, 1)), tolower(substring(x, 2)))
  #     return(title_case_string)
  #   }
  # }
  # for (i in 1:ncol(df_subset_char)) {
  #   df_subset_char[, i] <- sapply(df_subset_char[, i], toTitleCase)
  #   df_subset_char[, i] <- as.factor(df_subset_char[, i])
  # }
  df_subset<-list(df_subset_char,df_subset_num)
  return(df_subset)
}

checkLevels <- function(Trainsurv_subset,clinical_subset){
  for(i in 1:ncol(clinical_subset)){
    if(!all(levels(factor(clinical_subset[,i])) %in% levels(factor(Trainsurv_subset[,i])))){
      return(FALSE)
    }
  }
  return(TRUE)
...
remove_sparse <- function(df) {
    col_counts <- sapply(df, function(x) sum(!is.na(x)))
    remove_cols <- names(col_counts)[col_counts < 2]
    df[, !names(df) %in% remove_cols]
  }

metagene<-function(dat,gene,time,status){
  require(meta)
  require(survival)
  func<-function(dt,gene,time,status){
  # func<-function(dt,gene,time,status,set){
    if(gene %in% row.names(exprs(dt))==F){
      # message(paste(gene, "is not found in",paste0(dataset[i],","), " please check!!!"))
      return(NULL)
    }else if(!all(c(time,status) %in% names(pData(dt)))){
      # message( paste("Either the", time ,"or the", status,"was not found in the clinical data of the dataset", set,"you specified"))
      return(NULL)
    } else{
      message(dt@experimentData@url)
      expres<-exprs(dt)
      expres<-as.data.frame(expres)
      clinical<-pData(dt)
      clinical<-clinical[complete.cases(clinical[,time]) & complete.cases(clinical[,status]) & clinical[,time]>0,]
  ...
  res<-lapply(dat,FUN=func,gene=gene,time=time,status=status)
  # res<-list()
  # print(3)
  # for(i in 1:length(dat)){
  #   # message(names(dat)[i])
  #   print(names(dat)[i])
  #   res[[i]]<-func(dat[[i]],gene=gene,time=time,status=status,set=names(dat)[i])
  # }
  # print(1)
  # print(names(dat))
  # print(length(res))
  ...
  func2<-function(x){
    meta<-gen(es=x$HR,cil=x$lower95,ciu=x$upper95,studlab=row.names(x))
    return(meta)
  }
  meta<-func2(res)
}
...
pRRopheticPredict1<-function (testMatrix, drug, tissueType = "all", batchCorrect = "eb", 
          ...
          dataset = "cgp2014") 
{
  ...
  predictedPtype <- calcPhenotype1(cgpTrainData$trainDataOrd, 
...
calcPhenotype1<-function (trainingExprData, trainingPtype, testExprData, batchCorrect = "eb", 
          powerTransformPhenotype = TRUE, removeLowVaryingGenes = 0.2, 
          minNumSamples = 10, selection = -1, printOutput = TRUE, removeLowVaringGenesFrom = "homogenizeData") {
  if (all(class(testExprData) != "matrix")) 
    stop("ERROR: \"testExprData\" must be a matrix.")
  if (all(class(trainingExprData) != "matrix")) 
...
rmNA<-function(x,percent){
  tmp<-x
  if(any(rowMeans(is.na(tmp)) >percent)){
    tmp<-tmp[-which(rowMeans(is.na(tmp)) >percent), ]
  }else{
    tmp<-tmp
  }
  return(tmp)
}
# rmaexpression<-lapply(exp,rmNA,0.8)
checkNA<-function(x){
  NAs<-is.na(x)
  sumNAs<-sum(NAs)
  return(sumNAs)
...
imputeNA<-function(x){
  require(impute)
  # the impute of KnnImputation is gene expresion matrix, col is gene, and row is sample
  data<-t(x)
  mat<- impute.knn(data)
  rt<-mat$data
  data<-t(as.data.frame(rt))
  return(data)
}
...
validpred<-function(traindata,testdata,time,status,variable,ratio,bootstrap,time.inc,pred.time){
 ...
 testdata <- remove_sparse(testdata)
...
nestml<-function(expres,clinical,split,endpoint,ratio,ctrl,inner,outer,lnid,seed){
  ...
  if(split==T){
    ...
    testset<-expres[which(index==F),]
    trainsurv<-clinical[index,]
    testsurv<-clinical[which(index==F),]
...
cv<-function(expres,clinical,split,endpoint,ratio,lnid,inner,ctrl,rdesc,seed){
  ...
  if(split==F){
    ...
    if(finalmodel$id=="RandomForestSRC"){
      ...
      selvar<-selvar[selvar[,1]!=0,,drop=F]
      # listidx<-as.data.frame(selvar)[-1,]
      listidx<-as.data.frame(selvar)[-1,,drop=F]
      names(listidx)<-"coeff"
      selvar<-row.names(selvar)[-1]
