...
cancer <- reactive({
  # filter(dataset,Cancer == input$cancer)
  dataset[dataset$Cancer==input$cancer,]
})
...
Rawdata<-eventReactive(input$data,
                       ...
                       if(input$inputType == "Public dataset"){
                         #library(CBPhelper)
                         require(CuratedCancerPrognosisData)
                         require(Biobase)
                         #require(ggpubr)
                         #require(survival)
                         #require(psych)
                         ...
                         if(sum(is.na(expres))!=0){
                           expres<-rmNA(x=expres,percent=0.8)
                           expres<-imputeNA(expres)
...
observeEvent(input$data,{
  updateCollapse(session, "collapsedatainput", open = "Overview of discovery set", close = "Descriptions and parameters for data input")
...
cancer2 <- reactive({
  # filter(dataset,Cancer == input$cancer)
  dataset[dataset$Cancer==input$cancer2,]
})
...
Rawdata2<-eventReactive(input$data2,
                        ...
                        if(input$inputType2 == "Public dataset"){
                          ...
                          if(sum(is.na(expres2))!=0){
                            expres2<-rmNA(x=expres2,percent=0.8)
                            expres2<-imputeNA(expres2)
                            expres2<-as.data.frame(expres2)
                          }
