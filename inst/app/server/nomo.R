...
extvcrun<-eventReactive(input$exvalidbt,{
  ...
  clinfeature<-lapply(clinfeat,fc)
  res<-isolate({nestrun()})
  #Trainsurv<-res$Trainsurv
  Trainsurv_subset<-res$Trainsurv[,variable,drop=F]
  clinical_subset<-clinical[,variable,drop=F]
  ...
  if(is.numeric(OS.time)==F || is.numeric(OS)==F ){
    ...
    createAlert(
      ...
      title = "Please note!",
      style =  "danger",
      content = paste("Please check!",paste(setdiff(variable,names(res$Trainsurv)),collapse = ", "),"are not in the discovery set."),
      append = T,
      dismiss = T
    )
    return(NULL)
  }else if(!all(variable %in% names(res$Trainsurv))){
    createAlert(
      session,
      "exnomomess",
      "exampleAlert",
      title = "Please note!",
      style =  "danger",
      content = paste("Variable",paste(variable[!variable %in% names(res$Trainsurv)]),"you selected is not in the discovery set, please check!"),
      append = T,
      dismiss = T
    )
    return(NULL)
  } else if(checkmode(Trainsurv_subset,clinical_subset)==FALSE){
    createAlert(
      session,
      "exnomomess",
      "exampleAlert",
      title = "Please note!",
      style =  "danger",
      content = "Please check! At least one variables in the discovery set and external validation set are not the same type.",
      append = T,
      dismiss = T
    )
    return(NULL)
  } else {
    Trainsurv_subset<-transf(Trainsurv_subset)
    clinical_subset<-transf(clinical_subset)
    if(length(clinical_subset[[1]])!=0){
    # }
    if(checkLevels(Trainsurv_subset[[1]],clinical_subset[[1]])==FALSE){
      createAlert(
        session,
        "exnomomess",
        "exampleAlert",
        title = "Please note!",
        style =  "danger",
        content = "Please check! At least one variables in the discovery set and external validation set are not the same levels.",
        append = T,
        dismiss = T
      )
      return(NULL)
      ...
      withProgress(
        ...
        detail = "It may take a while, please be patient",
        value = 5,{
      clinical_subset<-as.data.frame(cbind(clinical_subset[[1]],clinical_subset[[2]]))
      if(identical(row.names(data$clinical),row.names(clinical_subset))){
        clinical_subset[,time]<-data$clinical[,time]
        clinical_subset[,status]<-data$clinical[,status]
      }
      data$clinical<-clinical_subset
      extvalid(
        extdata = data,
        variable = variable,
        res = res,
