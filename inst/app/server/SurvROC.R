...
# yearlabelx <- eventReactive(Rawdata(),{
yearlabelx <- reactive({
...
SurvROCplot<-eventReactive(input$SurvROCbt,{
  input$SurvROCbt
  data <- isolate({
    rawdata()
  })
...
observeEvent(input$SurvROCbt, {
  updateCollapse(session, "collapseSurvROC", open = "Time-dependent ROC analysis", close = "Descriptions and parameters")
})
