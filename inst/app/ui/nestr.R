tabItem(
  ...
  fluidPage(
    ...
    # status = "success",
    # bsAlert("nestrmess"),
    bsCollapse(id = "collapsenestr", open = "Descriptions and parameters for benchmark experiment",
               bsCollapsePanel("Descriptions and parameters for benchmark experiment",  includeHTML("nestr.html"), style = "default"),
               bsCollapsePanel("Benchmark experiment output",style = "default",
    tabBox(width = NULL,
    ...
    fluidRow(column(4,align="left",
                    hidden(div(id = "modelcomp_wrapper",
                               splitLayout(
                                 numericInput("modelcompwidthdl","Figure width",value = 5),
                                 numericInput("modelcompheightdl","Figure height",value = 5)),
                               downloadButton('downloadmodelcomp', 'Download figure', class = "butt2")
                    ))
    ))
  ),
    tabPanel("Biomarker output", DT::dataTableOutput('biomarkout'),
             useShinyjs(),
             fluidRow(column(4,align="left",
  ...
  column(3,
    ...
    box(title="Size control",solidHeader = F,collapsible = T,width = NULL,collapsed = T,
        sliderInput("nrwidth", "Width (%)", min = 0, max = 100, value = 45),
        sliderInput("nrheight", "Height (px)", min = 0, max = 1000, value = 350)
    ),
    actionButton("nrbt",
                 "Submit",
