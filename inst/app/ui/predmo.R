tabItem(tabName = "bmpm",
        fluidRow(column(9,
                        ...
                        #   align="center",
                          bsAlert("bmmess"),
                        bsCollapse(id = "collapsebmpm", open = "Descriptions and parameters",
                                   bsCollapsePanel("Descriptions and parameters",  includeHTML("premodel.html"), style = "default"),
                                   bsCollapsePanel("Prediction model",style = "default",
                        
                          tabBox(width = NULL,
                            tabPanel("Survival ROC curve",align="center", uiOutput('bmsurvROC'),
                                     useShinyjs(),
                                     fluidRow(column(4,align="left",
                                                     hidden(div(id = "bmsurvROC_wrapper",
                                                                splitLayout(
                                                                    numericInput("bmsurvROCwidthdl","Figure width",value = 10),
                                                                    numericInput("bmsurvROCheightdl","Figure height",value = 5)),
                                                                downloadButton('savebmsurvROC', 'Download figure', class = "butt2")
                                                                ))
                                     ))
                                     ),
                            tabPanel("KM plot",align="center", uiOutput('bmKMplot'),
                                     fluidRow(column(4,align="left",
                                                     hidden(div(id = "bmKMplot_wrapper",
                                                                splitLayout(
                                                                    numericInput("bmKMplotwidthdl","Figure width",value = 10),
                                                                    numericInput("bmKMplotheightdl","Figure height",value = 5)),
                                                                downloadButton('savebmKMplot', 'Download figure', class = "butt2")
                                                     ))
                                     ))
                                     ),
                            tabPanel("Forestplot",align="center",uiOutput('bmCoxforest'),
                                     fluidRow(column(4,align="left",
                            ...
                            tabPanel("CoxPH table", DT::dataTableOutput('bmCoxtable'),
                                     fluidRow(column(4,align="left",
        ...
        column(3,box(
          ...
          box(title="Survival ROC setting",solidHeader = F,collapsible = T,width = NULL,collapsed = F,
              ...
              sliderInput("bmSurvROCwidth","Plot Width (%)",min = 0,max = 100, value = 90),
              sliderInput("bmSurvROCheight","Plot Height (px)",min = 0,max = 1000,value = 400))
              ),
          ...
          sliderInput("bmsurvwidth", "Plot Width (%)", min = 0, max = 100, value = 90),
          sliderInput("bmsurvheight", "Plot Height (px)", min = 0, max = 1200, value = 400))
          ),
          box(title = "CoxPH model setting",solidHeader = F,collapsible = T,width = NULL,collapsed = F,
            ...
            box(title = "Size control",width = NULL,solidHeader = TRUE, collapsible = TRUE, collapsed = TRUE,
                sliderInput("bmCoxwidth", "Plot Width (%)", min = 0, max = 100, value = 80),
